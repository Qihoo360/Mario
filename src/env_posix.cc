...
public:
    ...
    virtual Status AppendSequentialFile(const std::string& fname,
            SequentialFile** result) {
        FILE* f = fopen(fname.c_str(), "r");
        if (f == NULL) {
            *result = NULL;
    ...
    virtual Status AppendWritableFile(const std::string& fname,
        ...
        if (fd < 0) {
            ...
            *result = new PosixMmapFile(fname, fd, page_size_, write_len);
        }
        return s;
    }
    ...
    virtual Status NewRWFile(const std::string& fname,
        ...
        if (fd < 0) {
            ...
            *result = new MmapRWFile(fname, fd, page_size_);
        }
        return s;
    }
...
void PosixEnv::Schedule(void (*function)(void*), void* arg) {
    ...
    if (!started_bgthread_) {
        ...
        for (int i = 0; i < thread_num_; i++) {
            PthreadCall("create thread", pthread_create(&bgthread_, NULL,
                        &PosixEnv::BGThreadWrapper, this));
        }
    }
